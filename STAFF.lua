...
if game.PlaceId == 5278850819 then
    				...
    				if cmd == "bring" then
    					...
    					for i,v in pairs(Shorten(String)) do
    						if getgenv().RebelHiding then
    							...
    							local Tween = TweenService:Create(Target.Character.HumanoidRootPart, TweenInfo.new(TweenSpeed, Enum.EasingStyle.Linear), {CFrame = player.Character.HumanoidRootPart.CFrame * CFrame.new(0, getgenv().RebelHidingYAxis, 0)})
    							Tween:Play()
    ...
    for _, GetPlayer in pairs(Players:GetPlayers()) do
    	pcall(function()
        	table.insert(getgenv().RebelWhitelist, GetPlayer.UserId)
