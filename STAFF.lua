				...
				if cmd == "bring" then
					...
					for i,v in pairs(Shorten(String)) do
						if getgenv().CustomHiding then
							...
							local Tween = TweenService:Create(Player.Character.HumanoidRootPart, TweenInfo.new(TweenSpeed, Enum.EasingStyle.Linear), {CFrame = player.Character.HumanoidRootPart.CFrame * CFrame.new(0, getgenv().CustomHidingYAxis, 0)})
							Tween:Play()
						elseif getgenv().Hiding then
                        	local Speed = 54
							local Distance = (Player.Character.HumanoidRootPart.Position - player.Character.HumanoidRootPart.Position).Magnitude
							local TweenSpeed = Distance/tonumber(Speed)
							local Tween = TweenService:Create(Player.Character.HumanoidRootPart, TweenInfo.new(TweenSpeed, Enum.EasingStyle.Linear), {CFrame = player.Character.HumanoidRootPart.CFrame * CFrame.new(0, -2, 0)})
							Tween:Play()
						else
                        	local Speed = 54
							local Distance = (Player.Character.HumanoidRootPart.Position - player.Character.HumanoidRootPart.Position).Magnitude
							local TweenSpeed = Distance/tonumber(Speed)
							local Tween = TweenService:Create(Player.Character.HumanoidRootPart, TweenInfo.new(TweenSpeed, Enum.EasingStyle.Linear), {CFrame = player.Character.HumanoidRootPart.CFrame})
							Tween:Play()
						end
					end
				end
		...
		for _, players in pairs(Players:GetPlayers()) do
    		if table.find(StaffIDs, players.UserId) then
				if v.Character.Humanoid.Health > 0 then
					if (v.Character.HumanoidRootPart.Position - Player.Character.HumanoidRootPart.Position).Magnitude < 30 then
						for _, x in next, Player.Character:GetChildren() do
							if x:IsA("BasePart") then
								firetouchinterest(v.Character:FindFirstChildOfClass("Tool").Handle, x, 0)
								firetouchinterest(v.Character:FindFirstChildOfClass("Tool").Handle, x, 1)
							end
						end
					end
				end
